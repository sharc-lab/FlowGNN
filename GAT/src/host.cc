...
aligned_vector<WT_TYPE> scoring_fn_target_fixed(NUM_LAYERS * NUM_HEADS * EMB_DIM);
aligned_vector<WT_TYPE> scoring_fn_source_fixed(NUM_LAYERS * NUM_HEADS * EMB_DIM);
aligned_vector<WT_TYPE> linear_proj_weights_fixed(NUM_LAYERS * NUM_HEADS * EMB_DIM * NUM_HEADS * EMB_DIM);
aligned_vector<WT_TYPE> skip_proj_weights_fixed(NUM_LAYERS * NUM_HEADS * EMB_DIM * NUM_HEADS * EMB_DIM);
...
int main(int argc, char **argv) {
    ...
    cl_int err;
    cl::Context context;
    cl::Kernel krnl_GAT_compute_graphs;
    cl::CommandQueue q;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_GAT_compute_graphs = cl::Kernel(program, "GAT_compute_graphs", &err));
            valid_device = true;
    ...
    printf("\n******* This is the HLS for GAT model *******\n");

    load_weights();
    printf("\n******* Weights loading done *******\n");

    OCL_CHECK(err, cl::Buffer scoring_fn_target_buf(context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        NUM_LAYERS * NUM_HEADS * EMB_DIM * sizeof(WT_TYPE),
        scoring_fn_target_fixed.data(),
        &err));
    OCL_CHECK(err, cl::Buffer scoring_fn_source_buf(context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        NUM_LAYERS * NUM_HEADS * EMB_DIM * sizeof(WT_TYPE),
        scoring_fn_source_fixed.data(),
        &err));
    OCL_CHECK(err, cl::Buffer linear_proj_weights_buf(context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        NUM_LAYERS * NUM_HEADS * EMB_DIM * NUM_HEADS * EMB_DIM * sizeof(WT_TYPE),
        linear_proj_weights_fixed.data(),
        &err));
    OCL_CHECK(err, cl::Buffer skip_proj_weights_buf(context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        NUM_LAYERS * NUM_HEADS * EMB_DIM * NUM_HEADS * EMB_DIM * sizeof(WT_TYPE),
        skip_proj_weights_fixed.data(),
        &err));
    OCL_CHECK(err, cl::Buffer graph_pred_weights_buf(context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        NUM_TASK * EMB_DIM * sizeof(WT_TYPE),
        graph_pred_weights_fixed.data(),
        &err));
    OCL_CHECK(err, cl::Buffer graph_pred_bias_buf(context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        NUM_TASK * sizeof(WT_TYPE),
        graph_pred_bias_fixed.data(),
        &err));
    ...
    int idx = 0;
    krnl_GAT_compute_graphs.setArg(idx++, num_of_graphs);
    krnl_GAT_compute_graphs.setArg(idx++, nums_of_nodes_buf);
    krnl_GAT_compute_graphs.setArg(idx++, nums_of_edges_buf);
    krnl_GAT_compute_graphs.setArg(idx++, reload_weights_buf);
    krnl_GAT_compute_graphs.setArg(idx++, result_buf);
    krnl_GAT_compute_graphs.setArg(idx++, node_feature_buf);
    krnl_GAT_compute_graphs.setArg(idx++, edge_list_buf);
    krnl_GAT_compute_graphs.setArg(idx++, scoring_fn_target_buf);
    krnl_GAT_compute_graphs.setArg(idx++, scoring_fn_source_buf);
    krnl_GAT_compute_graphs.setArg(idx++, linear_proj_weights_buf);
    krnl_GAT_compute_graphs.setArg(idx++, skip_proj_weights_buf);
